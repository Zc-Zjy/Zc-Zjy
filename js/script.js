...
(function($) {
	...
	var fn = {
		...
		motto: function() {
			...
			if (CONFIG.preview.motto.api) {
				var data_contents = CONFIG.preview.motto.data_contents && JSON.parse(CONFIG.preview.motto
					.data_contents);
	...
	var action = {
		...
		notification: function() {
			...
			var page_white_list = CONFIG.notification.page_white_list && JSON.parse(CONFIG.notification
				.page_white_list);
			var page_black_list = CONFIG.notification.page_black_list && JSON.parse(CONFIG.notification
				.page_black_list);
			var path = window.location.pathname;
			if ((page_white_list && page_white_list.indexOf(path) < 0) || (page_black_list &&
					page_black_list.indexOf(path) >= 0)) return;
			var delay = CONFIG.notification.delay;
			var list = JSON.parse(CONFIG.notification.list);
			var playList = list.filter(function(item) {
				return item.enable && ZHAOO.utils.isDuringDate(item.startTime, item.endTime) &&
					item;
			});
		...
		hitokoto: function() {
			$.ajax({
				...
				success: function(res) {
					action.printHitokoto(res.hitokoto);
				}
			})
		},
		// getThemeType: function() {
		// 	$.ajax({
		// 		url: "/getThemeType?mode=" + localStorage.getItem('color-mode'),
		// 		type: 'GET',
		// 		dataType: 'json',
		// 		success: function(res) {
	...
	// setIntervalFun = setInterval(action.hitokoto, 7000);
